# SAMO-IS
Surrogate Assisted Multiobjective Optimization using Improved Preselection
This directory contains the code to run surrogate assisted evolutionary 
algorithm with improved preselection (SAMO_IS).  The code solves constrained multi-objective minimization problems.
...
The surrogate assisted evolutionary algorithm code is provided as a SAMO_IS.p. Please run the startup.m. Change the parameters in params.m. Output corresponding to each run are stored in .mat format in "Results\problem-name\run-i". 
