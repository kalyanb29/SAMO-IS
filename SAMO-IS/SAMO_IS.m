...
if prob.ng > 0
    ...
    X_pop = X_pop(rankbest,:);
    F_pop = F_pop(rankbest,:);
    G_pop = G_pop(rankbest,:);
    CVpop = CVpop(rankbest,:);
else
...
for i=1:param.generations
    ...
    if ~isempty(X_child_all)
        [F_child_all,G_child_all,~,TFE]=func(X_child_all,TFE);
    else
        F_child_all = [];
        G_child_all = [];
    end
    if ~isempty(X_child_all)
        if ~isempty(G_child_all)
            CV_child_all = sum(max(G_child_all,0),2);
        else
            CV_child_all = zeros(size(X_child_all,1),1);
        end
    else
        CV_child_all = [];
    end
    ...
    X_child_all_all = [X_child_star;X_child_all;X_pop];
    F_child_all_all = [F_child_star;F_child_all;F_pop];
    G_child_all_all = [G_child_star;G_child_all;G_pop];
    CV_child_all_all = [CV_child_star;CV_child_all;CVpop];
    X_child_all1 = round(X_child_all_all*1e4)/1e4;
    [~,idunique] = unique(X_child_all1,'rows','stable');
    X_child_all_u = X_child_all_all(idunique,:);
    F_child_all_u = F_child_all_all(idunique,:);
    if ~isempty(G_child_all_all)
        G_child_all_u = G_child_all_all(idunique,:);
    end
    if ~isempty(CV_child_all_all)
        CV_child_all_u = CV_child_all_all(idunique,:);
    end
    rank_all_u = sort_best(F_child_all_u,CV_child_all_u);
    X_child_all_u = X_child_all_u(rank_all_u,:);
    X_child_remain = setdiff(X_child_all_u,[X_child_all;X_pop],'rows','stable');
    numevalchild = size(X_child_remain,1) - size(X_child_all,1);
    numchildx = min(param.pop_size,numevalchild);
    child_x = X_child_remain(1:numchildx,:);
    ...
    X_pop=[X_pop;X_child;X_child_all];F_pop=[F_pop;F_child;F_child_all];G_pop=[G_pop;G_child;G_child_all];
    [X_pop, F_pop, G_pop]=sort_pop(X_pop, F_pop, G_pop);
